# -*- coding: utf-8 -*-
"""Copy of Assignment - List Comprehension.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1mWhaTdMaZeAHZQRDTFoerGLO6U_-0N2b
...
predicted_house_prices = [float(format(0.05*housearea + 10,".2f")) for housearea in house_areas]
...
actual_house_price=list(house_area_price_dict.values())
# Zip the two price lists together
two_price_list=list(zip(predicted_house_prices,actual_house_price))
print(two_price_list)

# List of squared differences between each element of the two lists using list comprehension
squared_difference_in_prices=[(predicted_house_prices[i]**2-float(actual_house_price[i]**2))for i in range(len(predicted_house_prices))]
...
e = 2.718
sigmoid_of_x = [1/(1+e**(-(values))) for values in x]
print(sigmoid_of_x)

# Return a list of boolean values if the sigmoid_of_x is greater than 0 and less than 1
boolean=[True for values in sigmoid_of_x if values>0 and values<1]
print(boolean)


# Create a list of those values which are greater than 0.5
greater_than=[values for values in sigmoid_of_x if values>0.5]
print(greater_than)
...
words_ending_with_ing =[all_words_in_sentence[index] for index,words in enumerate(all_words_in_sentence) if all_words_in_sentence[index][-3:]=='ing']
...
# Your code here
total_stop_words=custom_stop_words+list(stop_words)
total_stop_words.sort()
...
without_stop_words=[words for words in words_in_sentence if words not in total_stop_words]
